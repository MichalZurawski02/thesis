...
function lcv(var, assignment, csp::CSP)
    return sort(no_value_heurestic(var, assignment, csp), by=val -> sum(
        val === unass_val
        for unass_var in csp.neighbors[var] 
        if !haskey(assignment, unass_var) 
        for unass_val in no_value_heurestic(unass_var, assignment, csp)
    ))
end
